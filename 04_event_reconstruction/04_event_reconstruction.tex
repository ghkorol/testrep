...
\begin{itemize}
 \item \textbf{Seed generation}: assuming helical path of the particles in the
 ...
 as an average value over all the events and is defined as a most luminous point.}). The hits for the seed generation are taken from the
 inner pixel detector layers only. All the combinations of 3 hits or 2 hits and a beam spot center are tried out and if the created 
 track-like object fulfills some minimum $p_{T}$, maximum beam spot impact parameter\footnote{An \textit{impact parameter} $d_{xy}$ of the track is a minimum
 distance of the track to certain vertex or beam spot in the transverse $x-y$ plane.} and correct charge distribution requirements, it is
 accepted to be an initial track candidate.
 %
