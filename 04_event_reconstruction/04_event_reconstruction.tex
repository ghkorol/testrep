...
task is to reconstruct the two neutrinos which are neutral and don't interact with detector material, thus don't provide any direct trace. 
The observation of the transverse energy imbalances points to the presence of undetected objects.

The sum of the momenta in transverse plane has to be zero. The \textit{missing transverse energy} $E_{T}^{miss}$ is introduced
to reestablish the transverse energy imbalance: 

\begin{align}
 \sum_{leptons} \vec{p}_{T}^{lepton} + \sum_{jets} \vec{p}_{T}^{jet} + \vec{E}^{miss}_{T} = 0, \\
 \vec{E}_{T}^{miss} = - \sum_{leptons} \vec{p}_{T}^{lepton} - \sum_{jets} \vec{p}_{T}^{jet}.
\end{align}

Thus the missing transverse energy is defined as a vectorial sum over all reconstructed objects in the
event and then taking the opposite of their azimuthal momentum two-vector \cite{CMS-PAS-PFT-09-001}.
