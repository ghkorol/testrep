...
task is to reconstruct the two neutrinos which are neutral and don't interact with detector material, thus don't provide any direct trace. The measured quantity to rely on is
observation of the transverse energy imbalances (missing transverse energy $E^{miss}_{T}$) which may point to the presence of undetected objects.
The missing transverse energy is reconstructed using the particle flow algorithm \cite{CMS-PAS-PFT-09-001} as a vectorial sum over all reconstructed objects in the
event and then taking the opposite of this azimuthal, momentum two-vector. The missing transverse energy is the module of this vector.
