...
missing transverse energy calculated only from the PF, \ref{eq:PFMET}.
...
%  \item [--] \textbf{Calorimeter noise removal}: Event with anomalous calorimeter noise are removed.
 ...
 %%%%%
 \item [--] \textbf{Jets selection}: The events which contain at least two jets with $p_{T} > 30$ GeV and $|\eta| \leq 2.4$ are accepted. It is natural to expect
 that the events with less than two jets will be dominated by the Drell-Yan background as the Drell-Yan process does not contain jets in final state. This can also
 be seen from the figure \ref{fig:jetMultiSel} with the jet multiplicity in the events. 
...
%  \end{figure}
 %
 \item [--] \textbf{$b$-jets selection}: An event has to contain at least one jet, tagged as originating from the $b$-quark with the CSVL \ref{ssec:bTag}. The multiplicity of the $b$-tagged
...
The figure \ref{fig:CPetaptLep} shows the lepton $\eta$ and $p_{T}$. The overall good agreement between the data and simulation shapes is observed even in the side $\eta$ regions
and for the higher $p_{T}$. The control distribution of mass of the dilepton (electron-muon) system is shown on the figure \ref{fig:CPmll}. The simulation and experimental data
agree well within the statistical uncertainties in the medium values of the mass.
...
jets $\eta$, $p_{T}$ and the jet multiplicity in the events. From these plots one can conclude that there is a MC excess for the jets 
with the transverse momentum higher than 150\;GeV. The simulation describes better the central rapidity ranges and the jet multiplicities
smaller than 5.
