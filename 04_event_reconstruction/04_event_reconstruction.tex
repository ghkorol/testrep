...
\begin{itemize}
 ...
 for the new hits and reestimating the track parameters respectively. The procedure is based on the Kalman filter\cite{Fruhwirth:1987fm}. On each consecutive
 layer all the hits from the 3 sigma region around the seed trajectory are taken and fitted with a Kalman filter. The best $chi^{2}$ hit is taken and the trajectory
...
of the detector. After an object is reconstructed in one iteration, all the clusters and tracks assigned to it are blinded for the further iterations.
...
\begin{itemize}
 ...
 from tracker to calorimeter clusters and to the muon systems. The clusters in calorimeter and tracks in the tracker which the muons are assigned to
