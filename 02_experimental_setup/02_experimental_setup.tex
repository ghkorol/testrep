...
reconstruction, which is crucial for any physical process, but also a precise $b$- and $\tau$ identification. This is reached with the help of the silicon
...
The 4T superconducting CMS solenoid, 13 m in length and 6 m in diameter, is shown in the Fig.\ref{fig:solenoid}. 
The flux is returned through a 10000 ton heavy yoke \cite{CMSatLHC}. A very strong magnet allows not only curving
high momentum muons for their better reconstruction, but also keeps soft particles with a small bending radius
inside the inner detector layers. This reduces occupancy inside the calorimeters leaving only higher
momentum particles there. A solenoid design allows a compact size of the powerful magnet.
