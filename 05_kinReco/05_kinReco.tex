...
This section describes the method used for the full kinematics reconstruction of this system. The analytical solution of the
kinematic equations (sec. \ref{sec:MatBg}) as well as a performance description of the method 
(sec. \ref{sec:SolSer} -- \ref{sec:kinRecPerf}) are revealed.
...
\begin{itemize}
 ...
 which will result in the impossibility to find a real solution of the equations.
 %
...
of the correction factors are shown in Fig. \ref{fig:fE}. The correction factors are determined from the signal MC simulation using jets and 
...
\end{subfigure}
\caption{Distributions of the energy resolution response used for the energy smearing in the kinematic
reconstruction of the top-quark kinematics. The distribution of the factor for jets is shown on the left and for
leptons (electrons and muons) on the right.}
\label{fig:fE}
\end{figure}
...
Here the $E^{miss \; from \, reco}_{T_{x,y}}$, $p^{jets \; reco}_{x,y}$, $p^{leptons\;reco}_{x,y}$ are the missing transverse energy and momenta
...
To obtain one $t\bar{t}$ pair out of this candidate variety, the following steps are followed:

\begin{itemize}

 \item [--] First, the single combination of leptons and jets is selected (getting rid of the combinatorics part $N_{jet}!$). The lepton-jet 
 combination with two $b$-tagged jets (see sec. \ref{ssec:bTag}) is prioritized. If there is further indeterminacy connected with the single 
 lepton-jet combination, then to each combination is a weight $\omega = \omega_{m^{\bar{l}b}} \cdot \omega_{m^{l\bar{b}}}$ is assigned. Here 
 ...
 particle level after all kinematic cuts described in chapter \ref{chapt:event_sel}. A combination of leptons and jets with the highest weight 
 is taken for the further analysis. This reduces the number of the possible candidates to maximum $100 \times 4$.
...
Fig. \ref{fig:EffSF} shows the efficiencies and scale factors (as defined in \ref{chapt:event_sel}) for the $t\bar{t}$ kinematic reconstruction procedure 
depending on jet multiplicity in the event, 
...
show a flat behavior depending on different variables, thus the value of 0.99 for the scale factor is used for the analysis. This value was determined 
...
The scatter plots in Fig. \ref{fig:ScatterPl} show the correlation between the kinematic variables after the selection and kinematic reconstruction in the simulated
...
The kinematic reconstruction method shows desirable results over the complete kinematic range of the top system. The Fig. \ref{fig:CPkinTop} shows
...
shows a good agreement between experimental data and simulation at the small and intermediate $p_{T}$. However the higher $p_{T}$ range is not well
...
\caption{Control distributions of the top quark $p_{T}$ (top left), rapidity (top right) and  $x_{1}$ in the events 
 ...
 and simulated distributions of signal and different background are plotted. The plots each show in the bottom panel the data-to-MC yield ratio distributions. 
...
experimental data and MC. The invariant mass of the $t\bar{t}$ system is well described by the simulation for the lower mass values. Significant deviations are
...
\caption{Control distributions of the $p_{T}$ (top left), rapidity (top right) and invariant mass of the $t\bar{t}$ system in the events 
 after the kinematic reconstruction. The experimental data with the error bars corresponding to the statistical uncertainties
 and simulated distributions of signal and different backgrounds are plotted. The plots each show in the bottom panel the data-to-MC yield ratio distributions.}
...
The results obtained in TOP-12-028 \cite{TWikiTOP12028} \cite{Khachatryan:2015oqa} exploiting the kinematic reconstruction are presented 
