...
Additionally to the energy variation a random gaussian smearing in a random direction of the azimuth angle $\theta$ (see sec. \ref{sec:CMS}) around the nominal direction is applied. The resolutions for
the gaussians are taken from the MC distributions presented on the Figure \ref{fig:dAngle}. The polar angle $\phi$ is taken randomly from 0 to $2\pi$.
