...
\begin{itemize}
 ...
 \item \textit{The $W^{\pm}$ masses} ($m_{W\pm}$) are assumed to be known and constrained to values randomly taken from the generator $W^{\pm}$ mass spectrum.
\end{itemize}
...
There can be up to four solutions for the equation \ref{eq:eqLSf}. The distribution of the number of solutions for the generated $t\bar{t}$ signal sample (\MG + \PYTHIA,
see chapt. \ref{chapt:MC}) is shown in Figure \ref{fig:LSNsol}. Two (four) solutions per event are expected in 80$\%$ (20\%) of the cases.
...
\begin{itemize}
 ...
 jets. However there is a priori no information if a jet comes from the $t$ or the $\bar{t}$. For this reason each jet is being paired to one of the leptons, and then to another
...
\begin{itemize}
 ...
 decays, respectively. These weights are calculated from the spectrum of the lepton-jet pairs in top decays obtained from the signal MC on the 
...
lepton kinematics and missing transverse energy. The integrated efficiency of the kinematic reconstruction method is 93 $\%$. Overall, the scale factors 
...
described by the MC. The rapidity is well described by the simulation in the central region, whereas for the edges MC slightly underestimates the data.
