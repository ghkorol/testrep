...
This chapter is an introduction to Elementary Particle Physics as described by the Standard Model.
First the particles are described, then the way they interact is shown. A short introduction
...
of atoms. The full picture of the atom structure was obtained after Ernest Rutherford's scattering experiment\cite{rutherford},
thus proving atoms to be non-elementary particles. It actually consists of a heavy positively charged core, called 
\textit{nucleus} and very light negatively charged electrons, moving around like satellites. 
The nucleus was later proven to be non-elementary. However, up to date no structure of electron was discovered 
...
antileptons -- negative ones. It is believed that the lepton number is conserved during the interactions.
...
As coupling constant is small ($\alpha \ll 1$), it can be used for the expansion in the perturbative calculations (observable ~ $\sum_{k=0}^{\infty} c_{k}\cdot\alpha^{k}$).
The naming of the processes lying beyond each of the terms of the expansion is
...
interacting particles. In the weak interaction, both the charge and the quark flavour exchange, are present. The lepton number and flavour both conserve
